...
UI_PATH_LOGIN = BASE_DIR / "ui" / "p.ui"
...
class LoginWindow(QMainWindow):
    """Simple login window loaded from ui/p.ui. On successful login opens inventory."""

    def __init__(self) -> None:
        super().__init__()
        uic.loadUi(str(UI_PATH_LOGIN), self)
        self.setWindowTitle("Login - Ngawi Library")
        self.loginButton.clicked.connect(self.handle_login)
        self.main_window: InventarisBukuApp | None = None

    def handle_login(self) -> None:
        username = self.usernameInput.text().strip()
        password = self.passwordInput.text().strip()
        if not username or not password:
            QMessageBox.warning(self, "Login", "Isi username dan password terlebih dahulu.")
            return

        # For now accept any non-empty credentials. Replace with real auth as needed.
        self.open_inventory()
